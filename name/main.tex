...
We use the temperature and salinity fields in MOM01 to calculate geostrophic velocity (see Section~\ref{Geostrophic velocity derivation}) to compare with CARS in \ref{Geostrophic velocity validation}. A short discussion of how a MOM01 transport budget derived from geostrophic velocity compares with the same budget derived from actual model velocity is presented in Section\,\ref{Southern Australia Current System Transport Budget}.
...
\end{equation}
%
These depth integrated velocity fields produce transports per unit width in \si{\square\meter\per\second}. Note that $\Vec{V\sub{up}}$ and $\Vec{V\sub{low}}$ from MOM01 are shown in Fig.\,\ref{f03_fig1_}c,d. In this case we simply used actual model velocity integrated in the upper and lower layers, rather than the sum of geostrophic and Ekman velocities. A comparison of MOM01 actual velocity with MOM01 geostrophic and Ekman velocity shows that the SBC and FC actual model velocities are generally stronger than the sum of the geostrophic and Ekman velocities (\ref{Geostrophic velocity validation}). Therefore the MOM01 actual SBC and FC transport will be stronger than MOM01 geostrophic transport (see Section\,\ref{Southern Australia Current System Transport Budget}).
%
\begin{figure}[p]
\centering
\includegraphics%
...
which we refer to as the SBC or FC transport, and the inward/outward transport across all volume boundaries which we refer to as the cross-boundary transport. The SBC transports include both geostrophic and Ekman drifts. The FC transports include geostrophic and Ekman drift in the upper layer south of the SBC, and only geostrophic transports in the lower layer (slope-FC and offshore-FC).
...
\subsection{Geostrophic velocity field validation} \label{Geostrophic velocity validation}
ddd

\subsection{Surface density field validation} \label{Annual-mean density field validation}
ddd

\subsection{Velocity field validation} \label{Velocity field validation}
ddd

\subsection{Velocity sections validation} \label{Velocity sections validation}
ddd
